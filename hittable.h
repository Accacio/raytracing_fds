...
hit_set_face_normal(hit_record * rec, ray ray, vec3 outward_normal)
{
    ...
    rec->normal = rec->front_face ? outward_normal : vec3multscalar(outward_normal, -1.);
}
...
int
hittable_hit(hittable hittable,
             ray ray,
...
int
hittable_list_hit(hittable * hit_list, int hit_list_size, ray ray, float t_min,float t_max,hit_record * rec)
{
    int hit_anything = 0;
